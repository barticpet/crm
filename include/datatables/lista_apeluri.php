...
if (isset($_REQUEST['status_contract']) && $_REQUEST['status_contract'] !=0 && $_REQUEST['status_contract'] !='x'){ 
        $editor->where( function ( $q ) { 
                $q->where( 'firma.firma_id', "(SELECT firma_id FROM status_contract WHERE status='".$_REQUEST['status_contract']."' AND last='1')", ' IN', false );
        } );
} elseif (isset($_REQUEST['status_contract']) && $_REQUEST['status_contract'] == '0'){ 
        //echo $_REQUEST['status_contract'].' - '.$_SESSION['user']['user_role'];
        $editor->where( function ( $q ) {
                $q->where( 'firma.firma_id', "(SELECT firma_id FROM status_contract WHERE last='1')", 'NOT IN', false );
        } );
}elseif ($_SESSION['user']['user_role']==4 || $_SESSION['user']['user_role']==2) { //echo $_REQUEST['status_contract'];
        $editor->where( function ( $q ) {
          $q->where( 'firma.firma_id', "(SELECT firma_id FROM status_contract WHERE last='1')", 'IN', false );
        } );
}

/*
if (isset($_REQUEST['status_contract']) && $_REQUEST['status_contract'] !=0){
        $editor->where( function ( $q ) {
                $q->where( 'firma.firma_id', "(SELECT firma_id FROM status_contract WHERE status='".$_REQUEST['status_contract']."' AND last='1')", ' IN', false );
        } );
} elseif ($_SESSION['user']['user_role']==4) {
        $editor->where( function ( $q ) {
                $q->where( 'firma.firma_id', "(SELECT firma_id FROM status_contract WHERE last='1')", 'NOT IN', false );
        } );
}*/
