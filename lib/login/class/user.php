...
class User{
    ...
    */
    public function login($email,$password){
        if(is_null($this->pdo)){
            ...
            $pdo = $this->pdo;
            $stmt = $pdo->prepare("SELECT id, fname, lname, email, wrong_logins, password, user_role FROM users WHERE email = ? and confirmed = '1' and active = '1' limit 1");
            $stmt->execute([$email]);
            $user = $stmt->fetch();
    ...
    */
    public function registration($email,$fname,$lname,$pass){
        ...
        $stmt = $pdo->prepare('INSERT INTO users (fname, lname, email, password,confirmed, confirm_code) VALUES (?, ?, ?,?, ?, ?)');
        if($stmt->execute([$fname,$lname,$email,$pass,1,$confCode])){
           $this->msg = 'Cont creat cu succes';
           return true;
    ...
    */
    ...
    {
        if (isset($_SESSION['user']) && !is_null($_SESSION['user']['id'])){
            $ip = $this->getIp();
            $query = $this->pdo->prepare("INSERT INTO user_attempts (user_id, ip) VALUES (?, ?)");
            return $query->execute(array($_SESSION['user']['id'], $ip));
        }
    }
